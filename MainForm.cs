...
internal sealed class MainForm : Form
{
    ...
    private static readonly GeneticRandomizationMode[] GeneticRandomizationModes =
    [
        new("Conservative 2-6", 2, 6, false),
        new("Full 1-10", 1, 10, true)
    ];
    ...
    private readonly Button _randomRawAppearanceButton = new();
    private readonly Button _resetAppearanceButton = new();
    private readonly Button _applyAppearanceButton = new();
    private readonly Button _applyMountDetailsButton = new();
    private readonly Button _applyGameStatsButton = new();
    private readonly Button _resetMountStatsButton = new();
    ...
    private readonly ComboBox _creatureGeneticsModePicker = new();
    ...
    private readonly Dictionary<int, CreatureAppearanceSnapshot> _loadedAppearanceSnapshots = new();
    private CreatureStatsSnapshot? _loadedStatsSnapshot;
    private readonly Dictionary<int, CreatureStatsSnapshot> _loadedStatsSnapshots = new();
    ...
    {
        TableLayoutPanel stats = new() { Dock = DockStyle.Top, ColumnCount = 5, RowCount = 7, Padding = new Padding(0, 10, 0, 0), AutoSize = true };
        for (int i = 0; i < 5; i++)
        {
        ...
        stats.RowStyles.Add(new RowStyle(SizeType.Absolute, 58));
        ...
        ConfigureButton(_resetMountStatsButton, "Reset Stats");
        _resetMountStatsButton.Click += (_, _) => ResetStatsToLoadedDefaults();
        stats.Controls.Add(_resetMountStatsButton, 4, 4);
        ConfigureButton(_applyGameStatsButton, "Baseline Stats");
        _applyGameStatsButton.Click += (_, _) => ApplyGameDerivedStats();
        stats.Controls.Add(_applyGameStatsButton, 0, 5);
        stats.SetColumnSpan(_applyGameStatsButton, 2);
        Label statsNote = new()
        {
            Text = "Baseline Stats applies decoded table values only. ICARUS may recalculate final values from level, genetics, lineage, gear, and modifiers after load.",
            Dock = DockStyle.Fill,
            ForeColor = Color.FromArgb(95, 76, 24),
            Font = new Font(Font, FontStyle.Bold),
            TextAlign = ContentAlignment.MiddleCenter,
            AutoSize = false,
            Padding = new Padding(8, 4, 8, 4)
        };
        stats.Controls.Add(statsNote, 0, 6);
        stats.SetColumnSpan(statsNote, 5);
        return stats;
    }
    ...
    private Control BuildCreatureAppearancePanel()
    {
        ...
        ConfigureButton(_randomRawAppearanceButton, "Random Raw Appearance");
        _randomRawAppearanceButton.Click += (_, _) => RandomizeRawAppearance();
        appearance.Controls.Add(_randomRawAppearanceButton, 0, 5);
        appearance.SetColumnSpan(_randomRawAppearanceButton, 4);

        ConfigureButton(_resetAppearanceButton, "Reset Appearance");
        _resetAppearanceButton.Click += (_, _) => ResetAppearanceToLoadedDefaults();
        ConfigureButton(_applyAppearanceButton, "Apply Appearance");
        _applyAppearanceButton.Click += (_, _) => ApplyAppearance();
        appearance.Controls.Add(_applyAppearanceButton, 0, 6);
        appearance.SetColumnSpan(_applyAppearanceButton, 2);
        appearance.Controls.Add(_resetAppearanceButton, 2, 6);
        appearance.SetColumnSpan(_resetAppearanceButton, 2);

        Label note = new()
        {
            Text = "Apply Appearance keeps changes in memory for this creature. Save writes all applied changes to Mounts.json.",
            Dock = DockStyle.Fill,
            TextAlign = ContentAlignment.MiddleLeft
        };
    ...
    private Control BuildCreatureGeneticsPanel()
    {
        ...
        _creatureGeneticsList.ColumnCount = 2;
        _creatureGeneticsList.RowCount = 0;
        _creatureGeneticsList.Padding = new Padding(0, 0, 0, 4);
        _creatureGeneticsList.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 50));
        _creatureGeneticsList.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 50));
        root.Controls.Add(_creatureGeneticsList, 0, 0);

        TableLayoutPanel footer = new() { Dock = DockStyle.Fill, ColumnCount = 4, RowCount = 1 };
        ...
        Label note = new()
        {
            Text = "Genetics use the saved Value field and are clamped from 0 to 10. Natural genotype rolls are documented as 1-10; full range is experimental.",
            Dock = DockStyle.Fill,
            TextAlign = ContentAlignment.MiddleLeft
        };
        footer.Controls.Add(note, 0, 0);
        _creatureGeneticsModePicker.Dock = DockStyle.Fill;
        _creatureGeneticsModePicker.DropDownStyle = ComboBoxStyle.DropDownList;
        foreach (GeneticRandomizationMode mode in GeneticRandomizationModes)
        {
            _creatureGeneticsModePicker.Items.Add(mode);
        }
        _creatureGeneticsModePicker.SelectedIndex = 0;
        footer.Controls.Add(_creatureGeneticsModePicker, 1, 0);
        ...
        footer.Controls.Add(_randomizeCreatureGeneticsButton, 2, 0);
        ...
        footer.Controls.Add(_resetCreatureGeneticsButton, 3, 0);
    ...
    private void LoadSelectedMountsFile()
    {
        ...
        try
        {
            _loadedAppearanceSnapshots.Clear();
            _loadedStatsSnapshots.Clear();
            if (File.Exists(path))
            {
    ...
    private void RefreshMountPicker()
    {
        ...
        _randomRawAppearanceButton.Enabled = false;
        _loadedAppearanceSnapshot = null;
        _loadedStatsSnapshot = null;
    ...
    private void LoadSelectedMount()
    {
        ...
        try
        {
            if (_selectedMount is not null)
            {
                ...
                int variationMaximum = _selectedMount.AppearanceVariationCount > 0
                    ? Math.Max(0, _selectedMount.AppearanceVariationCount - 1)
                    : 1000000;
                _mountVariationInput.Maximum = Math.Max(variationMaximum, _selectedMount.Variation ?? 0);
    ...
    private void CaptureLoadedCreatureDefaults()
    {
        ...
        if (!_loadedStatsSnapshots.TryGetValue(_selectedMount.Index, out CreatureStatsSnapshot? statsSnapshot))
        {
            statsSnapshot = new CreatureStatsSnapshot(
                _selectedMount.Name,
                _selectedMount.Lineage,
                _selectedMount.Level,
                _selectedMount.CurrentHealth ?? 0,
                _selectedMount.Stamina ?? 0,
                _selectedMount.FoodLevel ?? 0,
                _selectedMount.WaterLevel ?? 0,
                _selectedMount.OxygenLevel ?? 0);
            _loadedStatsSnapshots[_selectedMount.Index] = statsSnapshot;
        }

        _loadedStatsSnapshot = statsSnapshot;

        if (!_loadedAppearanceSnapshots.TryGetValue(_selectedMount.Index, out CreatureAppearanceSnapshot? snapshot))
        {
            snapshot = new CreatureAppearanceSnapshot(
            ...
            _loadedAppearanceSnapshots[_selectedMount.Index] = snapshot;
        }

        _loadedAppearanceSnapshot = snapshot;
    ...
    private void RefreshAppearanceVariantPicker()
    {
        ...
        try
        {
            ...
            _randomRawAppearanceButton.Enabled = _selectedMount.AppearanceVariationCount > 0;
    ...
    private void RefreshMountPickerLabel()
    {
        ...
        object? selectedItem = _mountPicker.Items[selectedIndex];
        _mountPicker.Items[selectedIndex] = selectedItem;
    }
    ...
    private void RefreshCreatureGeneticRows()
    {
        ...
        try
        {
            int geneticCount = _selectedMount.Genetics.Count;
            _creatureGeneticsList.RowCount = (int)Math.Ceiling(geneticCount / 2d);
            for (int i = 0; i < _creatureGeneticsList.RowCount; i++)
            {
                _creatureGeneticsList.RowStyles.Add(new RowStyle(SizeType.Absolute, 42));
            }
            ...
            foreach (CreatureGeneticEntry genetic in _selectedMount.Genetics)
            {
                ...
                AddCreatureGeneticEditorCell(index, genetic);
                index++;
            }
        }
        finally
    ...
    private void AddCreatureGeneticEditorCell(int index, CreatureGeneticEntry genetic)
    {
        ...
        cell.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 92));
        cell.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100));
        cell.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 64));
        ...
        cell.Controls.Add(nameLabel, 0, 0);
        cell.Controls.Add(valueSlider, 1, 0);
        cell.Controls.Add(valueInput, 2, 0);

        _creatureGeneticsList.Controls.Add(cell, index % 2, index / 2);
    }
    ...
    private void RandomizeCreatureGenetics()
    {
        ...
        GeneticRandomizationMode mode = _creatureGeneticsModePicker.SelectedItem as GeneticRandomizationMode
            ?? GeneticRandomizationModes[0];
        _selectedMount.RandomizeGenetics(mode.MinimumValue, mode.MaximumValue);

        RefreshCreatureGeneticRows();
        string modeNote = mode.Experimental ? " experimental" : "";
        SetStatus($"Randomized {IcarusMount.GeneticValueNames.Count:N0}{modeNote} genetic value(s) for {_selectedMount.Name} using {mode.DisplayName}. Save to write Mounts.json.");
    }
    ...
    private void RandomizeKnownAppearanceVariant()
    {
        ...
        _mountBreedColorPicker.SelectedItem = variant;
        _mountVariationInput.Value = Math.Clamp(variant.Variation, 0, decimal.ToInt32(_mountVariationInput.Maximum));
        SetStatus($"Selected random {_selectedMount.Name} breed/color draft: {variant.DisplayName}. Apply Appearance to keep it in memory.");
    }

    private void RandomizeRawAppearance()
    {
        if (_selectedMount is null)
        {
            SetStatus("Load a creature first.");
        ...
        int variationCount = _selectedMount.AppearanceVariationCount;
        if (variationCount <= 0)
        {
            SetStatus("No game-data appearance variation range is mapped for this creature.");
            return;
        }
        ...
        {
            _mountBreedColorPicker.SelectedIndex = -1;
            _mountVariationInput.Maximum = Math.Max(_mountVariationInput.Maximum, variationCount - 1);
            _mountVariationInput.Value = Random.Shared.Next(0, variationCount);
            _mountUniqueVariationInput.Value = 0;
            _mountCosmeticSkinInput.Value = 0;
            _mountAltCosmeticSkinInput.Value = -1;
        }
        finally
        {
            _loadingAppearanceVariants = false;
        ...
        SetStatus($"Selected random {_selectedMount.Name} raw appearance draft: variation {_mountVariationInput.Value} of 0-{variationCount - 1}. Apply Appearance to keep it in memory.");
    }
    ...
    private void ApplySelectedAppearanceVariant()
    {
        ...
        _mountVariationInput.Value = Math.Clamp(variant.Variation, 0, decimal.ToInt32(_mountVariationInput.Maximum));
        SetStatus($"Selected {_selectedMount.Name} breed/color draft: {variant.DisplayName}. Apply Appearance to keep it in memory.");
    }
    ...
    private void ApplyAppearance()
    {
        ...
        if (_mountBreedColorPicker.SelectedItem is CreatureAppearanceVariant variant)
        {
        ...
        RefreshAppearanceVariantPicker();
        UpdateMountInfoLabel();
        SetStatus($"Applied {_selectedMount.Name} appearance in memory. Save to write Mounts.json.");
    }
    ...
    private void ApplyMountDetails()
    {
        ...
        CreatureStatsSnapshot? loadedStatsSnapshot = _loadedStatsSnapshot;
        ...
        _loadedStatsSnapshot = loadedStatsSnapshot;
    ...
    private void ResetStatsToLoadedDefaults()
    {
        if (_selectedMount is null || _loadedStatsSnapshot is null)
        {
            SetStatus("No loaded stats snapshot for this creature.");
            return;
        }

        _selectedMount.SetName(_loadedStatsSnapshot.Name);
        _selectedMount.SetLineage(_loadedStatsSnapshot.Lineage);
        _selectedMount.Level = _loadedStatsSnapshot.Level;
        _selectedMount.SetCurrentHealth(_loadedStatsSnapshot.Health);
        _selectedMount.SetStamina(_loadedStatsSnapshot.Stamina);
        _selectedMount.SetFoodLevel(_loadedStatsSnapshot.Food);
        _selectedMount.SetWaterLevel(_loadedStatsSnapshot.Water);
        _selectedMount.SetOxygenLevel(_loadedStatsSnapshot.Oxygen);

        _mountNameText.Text = _loadedStatsSnapshot.Name;
        SelectLineage(_loadedStatsSnapshot.Lineage);
        _mountLevelInput.Value = Math.Clamp(_loadedStatsSnapshot.Level, 0, decimal.ToInt32(_mountLevelInput.Maximum));
        _mountHealthInput.Value = Math.Clamp(_loadedStatsSnapshot.Health, 0, decimal.ToInt32(_mountHealthInput.Maximum));
        _mountStaminaInput.Value = Math.Clamp(_loadedStatsSnapshot.Stamina, 0, decimal.ToInt32(_mountStaminaInput.Maximum));
        _mountFoodInput.Value = Math.Clamp(_loadedStatsSnapshot.Food, 0, decimal.ToInt32(_mountFoodInput.Maximum));
        _mountWaterInput.Value = Math.Clamp(_loadedStatsSnapshot.Water, 0, decimal.ToInt32(_mountWaterInput.Maximum));
        _mountOxygenInput.Value = Math.Clamp(_loadedStatsSnapshot.Oxygen, 0, decimal.ToInt32(_mountOxygenInput.Maximum));

        RefreshMountPickerLabel();
        UpdateMountInfoLabel();
        SetStatus($"Reset {_selectedMount.Name} stats to loaded Mounts.json values. Save to write changes.");
    }

    private void ApplyGameDerivedStats()
    {
        if (_selectedMount is null)
        {
            SetStatus("Load a creature first.");
        ...
        _selectedMount.ApplyGameDerivedStats();
        _mountHealthInput.Value = Math.Clamp(_selectedMount.CurrentHealth ?? 0, 0, decimal.ToInt32(_mountHealthInput.Maximum));
        _mountStaminaInput.Value = Math.Clamp(_selectedMount.Stamina ?? 0, 0, decimal.ToInt32(_mountStaminaInput.Maximum));
        _mountFoodInput.Value = Math.Clamp(_selectedMount.FoodLevel ?? 0, 0, decimal.ToInt32(_mountFoodInput.Maximum));
        _mountWaterInput.Value = Math.Clamp(_selectedMount.WaterLevel ?? 0, 0, decimal.ToInt32(_mountWaterInput.Maximum));
        _mountOxygenInput.Value = Math.Clamp(_selectedMount.OxygenLevel ?? 0, 0, decimal.ToInt32(_mountOxygenInput.Maximum));
        UpdateMountInfoLabel();
        SetStatus($"Applied baseline table stats for {_selectedMount.Name} at level {_selectedMount.Level}. Save to write Mounts.json.");
    }
...
internal sealed record CreatureStatsSnapshot(
    string Name,
    string Lineage,
    int Level,
    int Health,
...
internal sealed record GeneticRandomizationMode(string DisplayName, int MinimumValue, int MaximumValue, bool Experimental)
{
    public override string ToString() => DisplayName;
}
